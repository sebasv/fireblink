...
impl Controls {
    ...
    /// Apply a button gesture to the grid.
    ///
    /// - Scene: short = next, long = previous (keeps the pattern running)
    /// - Seed: short = next, long = previous (restarts with that pattern)
    ...
    pub fn press(&mut self, grid: &mut Grid, button: Button, press: Press) {
        match (button, press) {
            (Button::Scene, Press::Short) => {
                self.scene = self.scene.next();
                grid.set_scene(self.scene);
            }
            (Button::Scene, Press::Long) => {
                self.scene = self.scene.prev();
                grid.set_scene(self.scene);
            }
            (Button::Seed, Press::Short) => {
                self.seed = self.seed.next();
                grid.reseed(self.seed);
            }
            (Button::Seed, Press::Long) => {
                self.seed = self.seed.prev();
                grid.reseed(self.seed);
            }
            ...
            (Button::Action, Press::Long) => {
                grid.reseed(self.seed); // restart the current pattern
            }
        }
...
mod tests {
    ...
    fn scene_button_cycles_scenes_without_touching_the_seed() {
        ...
        assert_eq!(grid.config().seed, Seed::Acorn);
    }
    ...
    fn hud_clears_after_timeout() {
        ...
        let mut ctl = Controls::new(Scene::Fire, Seed::Empty);
        ctl.press(&mut grid, Button::Scene, Press::Short);
        assert!(
